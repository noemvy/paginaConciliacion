...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["grabar"])) {
    ...
    $sqlAnularCheque = "UPDATE cheques 
                        SET fecha_circulacion = '$circulacion' 
                        WHERE numero_cheque = '$numero_cheque'";
if ($conect->query($sqlAnularCheque) === TRUE) {
        echo '<script>alert("Fecha Circulacion agregada.")</script>';
        
        $numero_cheque = "";
        $circulacion ="";
    } else {
        echo "Error al ingresar la fecha de circulacion en el cheque: " . $conect->error;
    }
